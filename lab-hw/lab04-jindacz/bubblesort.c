...
void bubblesort(int* array, unsigned int size){
  // iterating through whole array
    for(int i=0; i < size-1; i++){
      // iterating through whole array to make sure adjcant element is sorted
        for(int j=0; j < size-1; j++){
            // compare the two adjacant element array[j] and array[j+1]
            if(array[j] > array[j+1]){
              // swap the address so that smaller before larger
