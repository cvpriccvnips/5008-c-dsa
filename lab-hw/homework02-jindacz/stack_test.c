...
int unitTest6(int status){
    int passed = 0;
    stack_t* stk = create_stack(1);
    stack_push(stk, 1);
    if (stk->count == stk->capacity){
        if (stack_push(stk, 1) == -1){
            passed = 1;
        }
    }
...
// Tests size after push to a full stack
...
// Tests pushing and poping
int unitTest8(int status){
    int passed = 0;
    stack_t* stk = create_stack(3);
    stack_push(stk, 1);
    stack_push(stk, 2);
    stack_push(stk, 3);
    if (stack_pop(stk) == 3){
...
// Test stack single push
int unitTest10(int status){
    int passed = 0;
    
    stack_t* test_s = create_stack(MAX_DEPTH);
    
    if(stack_push(test_s,1) == 0){
        passed = 1;
    }
...
int unitTest11(int status){
    ...
    stack_t* test_s = create_stack(MAX_DEPTH);
    stack_push(test_s, 11);
    if(stack_pop(test_s) == 11){
        passed = 1;
    }
